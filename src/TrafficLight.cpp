...
{
    //This method receive should use std::unique_lock<std::mutex> and _condition.wait() 
...
{
    //This method send should use the mechanisms std::lock_guard<std::mutex> 
...
{
    // In this method an infinite while-loop 
...
{
    // This is a function with an infinite loop that measures the time between two loop cycles 
    ...
    while(true)
    {
      ...
      interval = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now()-currenttime).count();      
